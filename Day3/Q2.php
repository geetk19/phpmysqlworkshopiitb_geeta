<?php
    ...
    if($_SERVER['REQUEST_METHOD']=="POST"){
        if(isset($_POST['user-name']) && isset($_POST['marks1']) &&  isset($_POST['marks2']) && isset($_POST['marks3']) && isset($_POST['marks4']) && isset($_POST['marks5'])){
            ...
            if($value['name']==$_POST['user-name']){
                $result = $conn->prepare("UPDATE class1 set sub1=?,sub2=?,sub3=?,sub4=?,sub5=?,total_obtained=?,percent=? where name=?");
                $result->bind_param('ssssssss',$_POST["marks1"],$_POST["marks2"],$_POST["marks3"],$_POST["marks4"],$_POST["marks5"],$total_obtained,$percent,$_POST["user-name"]);
                $result->execute();
                sleep(2);
                $fet_data = $conn->prepare("SELECT * FROM class1 WHERE name=?");
                $fet_data->bind_param('s',$_POST["user-name"]);
